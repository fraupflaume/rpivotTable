  ...
  callWithJQuery(function($) {
    return $.pivotUtilities.export_renderers = {
      "TSV Export": function(pivotData, opts) {
        ...
        gimme = true;
        tellMe = pivotData.allTotal.format(10.01); /* check locale number format */
        if(tellMe.toString().includes(',')){
          gimme = false;
        }
        ...
        for (k = 0, len2 = rowKeys.length; k < len2; k++) {
          ...
          for (m = 0, len4 = colKeys.length; m < len4; m++) {
            ...
            if (agg.value() !== null) {
              ag = agg.value();
              if(gimme === false) {
                ag = ag.toString().replace(/\./g, ','); /* comma decimal sep */
              }
              row.push(ag);
            } else {
              row.push("");
...
//# sourceMappingURL=export_renderers.min.js (not really, this is not a 'min'... sigh)
